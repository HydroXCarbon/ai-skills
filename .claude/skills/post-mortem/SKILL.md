...
argument-hint: "[ticket key or destination]"
---
...
- **ticket key** — JIRA (or equivalent) key for the source ticket, used as the
  default destination and cross-reference.
- **destination** — where the writeup goes, if not the source ticket.
...
If a required input from Step 1 is missing, ask via `AskUserQuestion` rather
