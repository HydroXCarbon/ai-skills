...
description: "Fan-out research with N parallel agents. Use when the user asks to research a topic deeply, wants consensus from multiple angles, or says 'deep research' / 'multi-agent research'. Spawns N=5+ researcher subagents in parallel, synthesizes findings, exports PDF to ./output/<slug>/."
---
...
Do **not** trigger for single-source lookups, factual questions answerable from the user's own files, or codebase exploration. Use this only for **multi-angle, parallel, fan-out** research.
...
Adapt these angles to the topic. If the user passed an explicit `--agents N`, use that count (minimum 3, maximum 10).

If the topic is ambiguous (e.g. "research transformers" could mean electrical or ML), ask once via `ask_question` to disambiguate before fanning out.

## Step 3 — Fan out: dispatch N researcher subagents in parallel

Use `invoke_subagent` with the `research` subagent type. **Dispatch all N agents in a single call with N parallel subagent entries** — that's the entire point of fan-out.
...
Once all N researchers return, synthesize:
...
8. **Raw Researcher Dumps** — one `### Researcher N — <angle>` subsection per agent, containing that agent's verbatim output
...
```
python3 .agents/skills/fan-out-research/scripts/fan-out-research.py \
...
- Always fan out with at least 3 parallel agents; default 5. Never run as a single-agent research.
...
- Always preserve raw researcher dumps as the final section inside the single `report.md`.
