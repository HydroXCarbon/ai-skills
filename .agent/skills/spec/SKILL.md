...
- **feature brief** — one or two sentences describing the feature, extracted from the user's prompt. If absent, ask.

## Step 1 — Validate brief and derive slug

- If no brief was provided: ask via `ask_question` for the feature brief in one or two sentences. Use the answer as the brief.
...
Create a markdown spec document and save it to `_specs/<feature-slug>.md`. Use the **exact structure** defined in `_specs/template.md`.
...
- Write the result to `_specs/<feature-slug>.md`. Do not add technical implementation details such as code examples.
...
If the brief is too vague to derive a meaningful slug or summary, ask via `ask_question`:

- **Restate the brief** — provide a clearer one-sentence description of the feature.
- **Pick a suggested slug** — offer 2–3 candidate slugs derived from the brief for the user to choose from.
...
- Never include code blocks, function signatures, file paths to be created/edited, library/framework choices, or other implementation details. Planning only.
- Never run `git add`, `git commit`, `git push`, or modify any file outside `_specs/<feature-slug>.md`.
- Reject `template` as a slug.
- Stop immediately if the working tree is dirty. Do not stash, do not `--force`.
